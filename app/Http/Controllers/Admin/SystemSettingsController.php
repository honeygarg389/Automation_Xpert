...
class SystemSettingsController extends Controller
{
    ...
    {
        $request->validate([
            // SEC-004. `svg` is deliberately absent. It USED to be listed here
            // and was harmless only by accident: Laravel 12's `image` rule
            // hardcodes jpg/jpeg/png/gif/bmp/webp and adds svg only when passed
            // `allow_svg`, so `image` overrode the `mimes` list. That is a
            // framework-version-dependent non-exploit, not a fix — on Laravel
            // <=10 the same two lines accepted SVG.
            //
            // Do NOT "restore" svg here, and do NOT add `allow_svg`. An SVG is
            // an executable document: served from the public disk it runs
            // script on this application's origin. UploadExtensionSpoofingTest
            // pins this so a framework change breaks a test, not the app.
        ...
        // SEC-004: extension from sniffed content, never from the client filename.
        $path = $sm->prefixedPath('branding/logo-'.Str::uuid().'.'.SafeUploadExtension::for($file));
    ...
    {
        $request->validate([
            // SEC-004. `svg` removed — and unlike the logo rule above, it was
            // genuinely reachable here: this rule uses `file`, not `image`, so
            // nothing overrode the `mimes` list and an SVG carrying <script>
            // was accepted and served from the app origin.
            'favicon' => ['required', 'file', 'mimes:png,jpg,jpeg,gif,ico,webp', 'max:512'],
        ]);
        ...
        // SEC-004: extension from sniffed content, never from the client filename.
        $path = $sm->prefixedPath('branding/favicon-'.Str::uuid().'.'.SafeUploadExtension::for($file));
        $sm->disk()->putFileAs(dirname($path), $file, basename($path));
