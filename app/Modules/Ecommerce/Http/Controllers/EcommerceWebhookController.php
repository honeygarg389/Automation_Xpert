...
class EcommerceWebhookController extends Controller
{
    ...
    public function woocommerce(Request $request, EcommerceStore $store): JsonResponse
    {
        ...
        // Woo signs deliveries with the secret we set at webhook creation. The
        // signature is mandatory: an absent header must never mean "skip the
        // check", and an empty secret must never be used as an HMAC key.
        $signature = (string) $request->header('x-wc-webhook-signature', '');
        $secret = (string) ($store->webhook_secret ?? '');

        if ($signature === '' || $secret === '') {
            Log::warning('ecommerce.webhook.woo.signature_missing', [
                'store' => $store->id,
