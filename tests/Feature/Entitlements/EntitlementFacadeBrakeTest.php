...
class EntitlementFacadeBrakeTest extends TestCase
{
    ...
    /**
     * ⚠️ THE DIVERGENCE, RECORDED AS A TEST RATHER THAN A COMMENT.
     *
     * `MediaService` used `User::effectiveSubscription()`; the resolver uses
     * `Client::activePlan()`. For a SELF-SERVE customer — billed through
     * `subscriptions`, with no `client_subscriptions` row — those disagree.
     *
     * Today it changes no observable, because `storage_gb` is absent from every
     * real plan and both sources return null. This test makes the key real to
     * expose what happens the moment BUG-025 is fixed.
     *
     * It asserts the CURRENT behaviour, whatever it is, so that the day someone
     * makes `storage_gb` a real key this test fails and forces the decision
     * rather than shipping a silent quota change.
     */
    #[Test]
    public function a_self_serve_customers_quota_source_diverges_once_the_key_is_real(): void
    {
        $plan = Plan::factory()->create(['limits' => ['storage_gb' => 25]]);
        ...
        $this->assertSame(25 * (1024 ** 3), $legacy,
            'Legacy reads effectiveSubscription(), which DOES find a self-serve plan.');

        $this->assertSame(1024 ** 3, $facade,
            'The facade reads activePlan(), which does NOT see a self-serve subscription, so '
            .'it falls through to 1 GB. This divergence is invisible today because storage_gb '
            .'is absent from every real plan — but it is real, and it is the fourth instance '
            .'of the one-concept-two-places trap. If this test fails, someone has changed a '
            .'plan source and must decide which one is correct.');
    }
