...
class WorkspaceScopeBypassGuardTest extends TestCase
{
    ...
    private const SANCTIONED = [
        ...
        'app/Modules/Restaurant/Models/LegalAcceptance.php' => 'currentFor() takes an explicit workspace_id and its own where() IS the boundary. Under the scope a null context reported EVERY workspace as not having accepted — measured false-negative on a real acceptance before the bypass was added. Fail-open; bypassed deliberately, same shape as UsageMeter::current().',
        'app/Modules/Restaurant/Models/RestaurantOutlet.php' => 'eligibleForNewConnection() takes an explicit workspace_id and its own where() IS the boundary. Under the scope a null context returned EMPTY for every workspace — measured before the bypass was added, which would have made every outlet look ineligible platform-wide. Fail-open; bypassed deliberately, same shape as LegalAcceptance::currentFor().',
        'app/Modules/Restaurant/Services/PosConnectionProvisioningService.php' => 'restoreConnection() resolves the connection\'s outlet by ID. PosConnection::outlet() is itself an unscoped relation, but its RELATED model (RestaurantOutlet) carries BelongsToWorkspace — which applies to any query for that model reached through another model\'s relation. With no ambient admin/tenant context the relation fails CLOSED to null even for a valid outlet_id — measured directly (a service-level call with no acting-as admin), same shape as eligibleForNewConnection().',
        ...
        'app/Modules/SmartQr/Http/Controllers/Admin/QrDashboardController.php' => 'The platform-wide overview aggregates Active/Inactive/Configured counts and the Recently Assigned list across every tenant; the scope fails CLOSED with no admin workspace context and every count would read zero.',
        ...
        'app/Modules/Restaurant/Http/Controllers/Public/PublicRestaurantBillProfileController.php' => 'The public bill-profile route uses the same server-generated 256-bit opaque token to discover exactly one bill/workspace. It then establishes WorkspaceContext before loading or updating only that bill\'s already-linked contact; the browser cannot supply a contact or workspace id.',
