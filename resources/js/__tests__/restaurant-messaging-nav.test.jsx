...
vi.mock('react-i18next', () => ({
    useTranslation: () => ({ t: (key) => ({
        ...
        'nav.restaurant_branding': 'Restaurant Branding',
    })[key] ?? key }),
}));
...
describe('Restaurant Messaging navigation', () => {
    ...
    it('does not expose the owner-only settings page to client staff', () => {
        ...
        expect(messaging.items.some((item) => item.label === 'Restaurant Branding')).toBe(false);
    });

    it('shows Restaurant Branding beside Restaurant Messaging for workspace administrators', () => {
        const { result } = renderHook(() => useClientNav());
        const messaging = result.current.find((group) => group.label === 'Messaging');
        const item = messaging.items.find((candidate) => candidate.label === 'Restaurant Branding');

        expect(item).toMatchObject({
            href: '/client.restaurant.branding.index',
